# fix-39b94a0c516ccc0c — E3T1 test spec expects "Percent alerts coming soon" stub, but full percent alert flow is implemented

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The E3T1 test spec (`tests/specs/E3T1.json`, test "percent alert shows coming soon notification") expects that clicking `alert:type:percent` shows `answerCallbackQuery({ text: "Percent alerts coming soon." })` and does not navigate into the percent alert creation flow. However, the code at `src/bot.ts:1105-1110` routes `alert:type:percent` directly into the full percent alert creation flow (setting `alertStep = "pctCoin"` and showing the coin selection screen). This test spec was written when E3T1 was standalone (before E3T2 was merged), and now conflicts with the implemented E3T2 feature. The test would fail.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-39b94a0c516ccc0c.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-39b94a0c516ccc0c.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-39b94a0c516ccc0c.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
