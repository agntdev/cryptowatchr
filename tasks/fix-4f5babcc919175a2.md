# fix-4f5babcc919175a2 — Duplicate coin-ID-to-ticker maps risk divergence

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

`COIN_ID_TO_TICKER` in `src/price.ts:31-149` (coinId → ticker) and `KNOWN_COINS` in `src/bot.ts:77-195` (ticker → coinId) are separately maintained inverse maps. `coinIdForTicker` in bot.ts uses `KNOWN_COINS`, while `tickerToCoinId` in price.ts uses `COIN_ID_TO_TICKER`. If the two maps fall out of sync, a ticker recognized by `/price` could be rejected by watchlist/alert flows (or vice versa). Currently they appear consistent, but there is no compile-time or runtime check to detect divergence.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-4f5babcc919175a2.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-4f5babcc919175a2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-4f5babcc919175a2.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
