# fix-e84609f6c0b49784 — Alert message omits absolute change (E5T4 spec violation)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

E5T4 requires alert messages to include "coin, old price, new price, absolute and percent change, and the rule that triggered the alert." The `formatAlertMessage` function in `src/evaluator.ts:70-92` includes coin, old price, new price, percent change, and rule type, but never uses the `absChange` field from the `TriggeredAlert` interface (`evaluator.ts:11`). The absolute price change (e.g., `+$700.00` or `-$250.00`) is computed and stored in the `TriggeredAlert` object but never rendered into the message text.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-e84609f6c0b49784.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-e84609f6c0b49784.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-e84609f6c0b49784.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
