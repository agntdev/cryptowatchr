# fix-d66bf36ebd4776a1 — E6T1: recordUserActivity() and incrementAlertFireCount() are never called

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The `recordUserActivity(userId)` and `incrementAlertFireCount(ruleId)` methods are defined in the `PersistentStore` interface and implemented in all three backends (`RedisStore`, `MemoryStore`, `PostgresStore` in `src/store.ts`), but they are **never called** anywhere in the codebase. `recordUserActivity` is not invoked when users interact with the bot (e.g., in any command or callback handler in `bot.ts`). `incrementAlertFireCount` is not called in `poller.ts` or `evaluator.ts` when alerts fire. This means `getAdminStats()` would return `activeUsers30d=0` and `topFiredRules=[]` in all cases, rendering the admin stats feature useless even after adding the command handler.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-d66bf36ebd4776a1.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-d66bf36ebd4776a1.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-d66bf36ebd4776a1.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
