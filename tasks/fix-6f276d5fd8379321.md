# fix-6f276d5fd8379321 — .env.example missing DATABASE_URL and REDIS_URL

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The store selection logic in `createStore()` (store.ts:868-876) checks `DATABASE_URL` and `REDIS_URL` env vars to select the persistence backend, and the toolkit session storage (toolkit/session/redis.ts:97-105) checks `REDIS_URL`. Neither variable appears in `.env.example`, making it harder for operators to discover required/optional configuration for production deployment.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-6f276d5fd8379321.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-6f276d5fd8379321.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-6f276d5fd8379321.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
