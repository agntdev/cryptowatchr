# fix-8887b4d5ba98d118 — E7T2 test specs reference non-existent My Alerts main-menu button

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The dialog tests in `tests/specs/E7T2.json` (lines 33-34) expect the main menu `inline_keyboard` to contain a `menu:myalerts` button (`"My Alerts"`) on the second row alongside "Create Alert". However, `mainMenu()` in `src/bot.ts:379-391` returns only 6 buttons (Add Coin, My Watchlist, Create Alert, Price Check, Settings, Help) — no "My Alerts" button exists. The `menu:myalerts` callback handler at `src/bot.ts:1068-1086` **is** implemented and functional, but it is unreachable from the main menu UI. This makes the handler dead code and the test spec will never pass against the current implementation.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-8887b4d5ba98d118.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-8887b4d5ba98d118.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-8887b4d5ba98d118.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
