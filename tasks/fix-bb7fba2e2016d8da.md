# fix-bb7fba2e2016d8da — Missing /admin_stats command and owner authorization (spec violation)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The spec explicitly requires a `/admin_stats` command accessible to the configured owner (`TELEGRAM_OWNER_ID`). The bot has no handler for this command — any attempt to use it returns "Unknown command" (bot.ts:1442). Furthermore, the data pipeline for this feature is entirely dead:

- `recordUserActivity()` is defined in all three store implementations but **never called** anywhere — so active-user counts will always be zero.
- `incrementAlertFireCount()` is defined in all three store implementations but **never called** anywhere — so `topFiredRules` in admin stats will always be empty.
- `TELEGRAM_OWNER_ID` is not read from the environment or checked anywhere.

This means the entire admin-view feature referenced in the spec is non-functional.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-bb7fba2e2016d8da.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-bb7fba2e2016d8da.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-bb7fba2e2016d8da.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
