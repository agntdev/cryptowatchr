# fix-11a912080322ac61 — Settings menu missing timezone / quiet-hours configuration — onboarding flow unreachable

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The `/start` command in `src/bot.ts:706` now immediately shows the main menu (`MAIN_MENU_TEXT`) without triggering the onboarding flow. The Settings menu (`menu:settings`, line 1358) only exposes morning-summary controls — there is **no UI path** for a user to set their timezone or quiet hours.

The only code paths that call `store.setTimezone(...)` and `store.setQuietHours(...)` live inside handlers that depend on session states (`onboardingStep === "timezone"`, `quietHoursStep === "start"/"end"`) which `/start` never initializes. The callback handlers `onboard:tz:*`, `onboard:skip`, `onboard:qhours`, `onboard:done`, and the free-text timezone/quiet-hours message handlers are all dead code in the current `/start` → main-menu flow.

**Impact:** The persistent store correctly implements `setTimezone`, `getTimezone`, `setQuietHours`, `getQuietHours` and `deleteQuietHours`, but they are unreachable from the bot's command surface. The spec tests for E8T1 exercise onboarding flows that no longer match what `/start` presents to a real user.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-11a912080322ac61.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-11a912080322ac61.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-11a912080322ac61.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
