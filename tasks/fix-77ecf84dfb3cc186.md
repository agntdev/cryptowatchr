# fix-77ecf84dfb3cc186 — E7T1 test spec file is empty

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

`tests/specs/E7T1.json` is an empty array `[]`. The E7T1 task spec explicitly requires writing dialog tests: "Write tests to tests/specs/E7T1.json". While E7T1 (silent retries for the poller) has no user-facing dialog behavior to test, an empty file without explanation is ambiguous — it could mean tests were forgotten rather than intentionally omitted.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-77ecf84dfb3cc186.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-77ecf84dfb3cc186.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-77ecf84dfb3cc186.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
