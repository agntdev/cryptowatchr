# fix-d1e481df7c951395 — PRICEFIX-003 ticker validation runs after price already shown

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

The auto-loader in `buildBot()` loads handler files alphabetically (via `.sort()` on `readdirSync` results). This means PRICEFIX-002's `/price` handler fires BEFORE PRICEFIX-003's `/price` handler. PRICEFIX-002 fetches and displays prices, then PRICEFIX-003 validates the watchlist for invalid tickers and sends a separate warning message.

The spec states: "Show user warnings about removed tickers during /price command execution" — implying the warning should be part of the price output or shown before it. Instead, the warning arrives as a separate, out-of-order message after the price data has already been displayed.

**Severity note:** This is only triggered when invalid tickers actually exist in the watchlist.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-d1e481df7c951395.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-d1e481df7c951395.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-d1e481df7c951395.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
