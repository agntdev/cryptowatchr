# fix-2a7857258fd83ee0 — Custom ticker input in alert creation flows not validated against KNOWN_COINS

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

When a user enters a custom ticker for alert creation:

- **Threshold alerts** (`src/bot.ts:1550-1562`): the `alertStep === "coin"` handler accepts any string of 2+ characters without calling `coinIdForTicker()` or validating against `KNOWN_COINS`.
- **Percent alerts** (`src/bot.ts:1604-1617`): the `alertStep === "pctCoin"` handler similarly accepts any 2+ character string without coin validation.

The alert is saved with the raw ticker string. The evaluator (`src/evaluator.ts:159-160`) silently skips unknown coins with `if (!coinId) continue;`, meaning alerts created for unrecognized tickers will never trigger and the user receives no indication that the alert is non-functional.

Compare with `/price` command handler (`src/bot.ts:755-761`) which properly validates tickers before making API calls. The custom ticker alert flow should validate with `coinIdForTicker()` and show an error for unknown tickers.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-2a7857258fd83ee0.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-2a7857258fd83ee0.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-2a7857258fd83ee0.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
