...
export default function (store: PersistentStore): Composer<BotContext<Session>> {
  ...
  composer.callbackQuery("menu:price", async (ctx) => {
    ...
    try {
      ...
      await ctx.answerCallbackQuery({ text: priceCallbackText("unknown") });
      return;
    }
    ...
    try {
      const coinIds = [...new Set(entries.map((e: { coinId: string }) => e.coinId))];
      const data = await fetchPrices(coinIds);
      const text = formatPriceDisplay(data, entries.map((e: { ticker: string; coinId: string }) => ({ ticker: e.ticker, coinId: e.coinId })));
      await ctx.answerCallbackQuery();
      await ctx.editMessageText(text, { reply_markup: mainMenu() });
    } catch (err) {
      const kind = err instanceof PriceFetchError ? err.kind : "unknown";
      await ctx.answerCallbackQuery({ text: priceCallbackText(kind) });
      try {
        await ctx.editMessageText(priceErrorMessage(kind), { reply_markup: mainMenu() });
      } catch {
        // message may be unmodified; best-effort edit
      }
    }
  });
